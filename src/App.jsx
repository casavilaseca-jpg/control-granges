    ...
    return (
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 80 }}>
        ...
        <div style={{ padding: "12px 12px 0" }}>
          {tabLot === "resum" && (
            <div>
              {lot.entrades.map((e, i) => (<div key={e.id} style={{ background: "var(--color-background-secondary)", borderRadius: 12, padding: "12px 14px", marginBottom: 8 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}><span style={{ fontWeight: 600, fontSize: 14 }}>Entrada {i + 1}</span><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 12, color: "#888" }}>{e.data}</span>{lot.estat === "obert" && <button onClick={() => { setEditantEntrada(e); setModal("editarEntrada"); }} style={{ border: "none", background: "transparent", color: "#bbb", fontSize: 14, cursor: "pointer", padding: 0 }} title="Editar">✏️</button>}</div></div><div style={{ fontSize: 13, color: "#555" }}>{e.caps} caps · {e.pesKg > 0 ? `${e.pesKg.toLocaleString()} kg · ${(e.pesKg / e.caps).toFixed(1)} kg/cap` : <span style={{ color: "#f59e0b", fontWeight: 600 }}>Pes pendent ⚠️</span>}</div>{e.origen && <div style={{ fontSize: 12, color: "#888", marginTop: 3 }}>📦 {e.origen}</div>}</div>))}
